...
{
    int apps = atoi(argv[1]);  // I count on argv not containing "mpirun -np ?"
...
*/
        ...
        // exclusive or is the ^ sign
        if(type == 1)
        {
            double sigmaR = 0.0;
            for(int j=myrank+1; j<=n; j+=nprocs){   // set j=d fra 0 til <= n=log_2 (P-1)
                double vj = pow(j,-2.0);
                sigmaR += vj;
            }
...
*/
        ...
        if(type == 1)
        {
            ...
            double piM = 4.0*(4.0*sigmaM5 - sigmaM239);      //double piM = 4.0*(4.0*SM5 - SM239);
            double error;
            if (myrank==0) {
                double duration = MPI_Wtime() - time_start;
                double error = fabs(piM-pi);
